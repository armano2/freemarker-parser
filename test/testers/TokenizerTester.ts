...
export default class Tester {
  ...
  public hasNoParams() {
    if (this.token.params) {
      assert.strictEqual(
        this.token.params.length,
        0,
        `[${this.index}] Expected to have no params`,
      );
    }
    return this;
  }
  ...
  public hasParams(params?: string) {
    assert.strictEqual(
      this.token.params,
      params,
      `[${this.index}] Found ${this.token.params ||
        'no params'} but expected ${params || 'no params'}`,
    );
    return this;
  }
